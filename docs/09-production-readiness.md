...
Phases 1, 2, 3 e 5 estão fechadas para o baseline atual:
contratos/documentação foram reconciliados, lifecycle de jobs e restart recovery
têm regressões direcionadas,
a sanitização de erros/logs foi auditada nos caminhos conhecidos e o gate default
local/CI está limpo. Isso ainda **não** fecha produção privada completa: faltam
ensaios operacionais reais de Phase 4/8 — systemd, backup/restore, rollback e
recovery — registrados como evidência executada. Uso público ou multiusuário
...
| Observabilidade operacional | `/healthcheck`, `/lasterror`, `operational_errors.jsonl` e `execution_audit.jsonl` implementados; documentação/runbook criados | `HealthCheckService`, `LastErrorService`, `ExecutionAuditLogger`, `docs/03-manual-de-uso.md`, `docs/11-operator-runbook.md` | Falta ensaio real em host systemd; logs Python continuam em texto e não há métricas/alertas externos. |
| Operação e recovery | Runbook orientado a systemd criado para backup/restore, upgrade/rollback, recovery de `delivery_failed`/restart, modelos/cache e limpeza emergencial | `docs/11-operator-runbook.md`, `deploy/yt-transcriber-bot.service` | Procedimentos são documentados, mas smoke de systemd, ensaio de restore, rollback e recovery ainda não foram registrados como evidência executada. |
...
| 4 — Observabilidade e runbooks | Operação/docs | Implementada documentalmente; aguardando ensaios reais | `/healthcheck`, `/lasterror`, `operational_errors.jsonl`, `execution_audit.jsonl`, README/manual atualizado e `docs/11-operator-runbook.md` | Falta evidência de ensaio real documentado de systemd, backup/restore, rollback e recovery; `LOG_FORMAT=json` e alertas externos ficam como evolução futura. |
...
| 8 — Deployment systemd | Operação/docs | Pendente de ensaios reais | Evidência de smoke systemd, backup/restore, rollback e logs em host/staging real | Docker Compose permanece opcional/futuro. |
...
- [ ] Declarar produção privada/single-operator completa somente depois dos
      ...
      novas alterações.
- [ ] Executar e registrar ensaio real de backup/restore em host ou ambiente de
      staging.
- [ ] Executar e registrar smoke real de systemd start/stop/restart/rollback.
- [ ] Executar e registrar ensaio de recovery de `delivery_failed` e job
...
Atualize esta seção quando os checks forem executados. Evidência combinada mais
recente validada em **2026-07-09**:
...
- Executar e registrar ensaio real de backup/restore em host ou staging.
- Executar e registrar smoke real de systemd start/stop/restart/rollback.
- Executar e registrar ensaio de recovery de `delivery_failed` e job
  interrompido com os procedimentos do runbook.
- Reexecutar o baseline default antes de commit/release se houver novas
  alterações depois das evidências de 2026-07-09.
