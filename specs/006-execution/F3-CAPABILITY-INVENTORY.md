...
surfaces after the PLAN-003 capability migrations through TASK-P03-013. It is a
...
|---|---|---|---|
| `audio_converter.py` | Convert audio needed by approved workflows | Purpose-specific capability | P03-013 removed unused Telegram-specific split surface; keep conversion capability |
...
support/convergence evidence for `REQ-ARC-012`; P03-013 completed the
cross-cutting capability-port closure.
...
- the retired `FileStorage` / `LocalFileStorage` runtime surface does not
  reappear;
- application-port operation names do not leak transport/provider-specific
  implementation vocabulary;
- every abstract application-port contract in the explicit inventory can be
  implemented by a plain infrastructure-free test double.
